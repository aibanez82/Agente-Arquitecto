...
- Aplicado por Agente n8n: commit `d3e9307` (rama `stg`, repo `Agente-n8n`) — confirmado en vivo por el Arquitecto contra la instancia de n8n de STG (`placasRegex = /^[A-Z0-9]{6,7}$/` en el JSON real). Desplegado a **PRODUCCIÓN** el mismo 9 jul, según reporte de Agente n8n (PUT→200, activate→200, `webhookId` sin cambios `18c1b498-...`, 62 nodos, `AI Agent.systemMessage` actualizado a "6-7 caracteres" en sus 2 menciones, `Phone Number ID Guard` del Bug #15 intacto) — **sin verificación independiente del Arquitecto en PROD** (no hay credencial de API de n8n de producción en este entorno, a diferencia de STG).

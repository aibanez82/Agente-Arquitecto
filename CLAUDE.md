...
**🔴 Bloqueador nuevo del E2E (7 jul): drift de schema en `whatsapp_sessions` de staging.** Primer "hola" falla en el nodo `Load Session` con `column "rate_limit_data" does not exist`. **Confirmado que NO es una migración Django pendiente** — `whatsapp_sessions` no es una tabla gestionada por Django (sin modelo, sin migración; Django solo la toca con un INSERT SQL crudo de 7 columnas). La columna se agregó a prod por fuera del sistema de migraciones. Fix = `ALTER TABLE whatsapp_sessions ADD COLUMN rate_limit_data jsonb DEFAULT '{}'::jsonb;` directo en la BD de staging (no `manage.py migrate`, no depende de Juan si el Agente n8n tiene `STG_DATABASE_URL` con permisos). Prod tiene 13 columnas en esa tabla; recomendado diff completo contra staging antes de re-correr el E2E para no toparse con la siguiente columna faltante una por una. Detalle: `docs/2026-07-07-respuesta-agente-n8n-rate-limit-data-no-es-migracion.md`.

...
**Seguimiento automático de leads estancados (15-16 jul, nueva):** 7 checkpoints derivados en vivo de `conversation_phase`/`captured_data`, hasta 3 reintentos, sin campo de estado nuevo. Diseño acordado con Juan como "parche temporal" (su arquitectura grande de `Lead.estado` queda para después). n8n listo en STG (auth agregada al webhook `proactive-wa-message`), **Django sin arrancar**. Riesgos abiertos: solapamiento con el scheduler de follow-up de 15 min ya existente (Bug #13, Issue #74) y el contrato `session_id` debe empezar con `52` del mismo webhook que usa el botón "Tomar conversación" del Dashboard. Detalle: `docs/iniciativas/seguimiento-leads-estancados.md`.

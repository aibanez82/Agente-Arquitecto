...
| 14 | El AI Agent a veces responde con el mensaje de "fuera de alcance" (`"Solo puedo ayudarte con la contratación de tu póliza de auto. ¿Seguimos con la contratación?"`, de la SECURITY RULE #4 del systemMessage) ante mensajes que SÍ están en alcance — incluida una simple confirmación **"sí"** justo después de mostrar el resumen completo de la póliza (fase `summary_confirmation`), bloqueando la emisión en loop. **Confirmado en producción, con un cliente real (7 jul):** lead 753 / cotización 2205 (BK Encore 2019) escribió *"Solo quería cotizar porque el seguro que tengo se vence en un mes"* (mensaje benigno y en alcance) y el bot respondió con el mensaje de fuera-de-alcance; el cliente contestó *"Por ahora no gracias"* y el bot lo dejó ir sin insistir — **posible conversión perdida real**. Reproducido también en vivo por Alberto durante la validación E2E del Bug #10 en staging: tras el resumen de póliza (Honda Accord 2026), "sí" disparó el mismo mensaje en loop, bloqueando la Prueba B (serie válida) de esa validación. **Hipótesis sin confirmar:** el prompt mezcla las SECURITY RULES (máxima prioridad, con esa frase textual) con las instrucciones de fase más abajo; ante inputs cortos/ambiguos el modelo puede estar priorizando la regla de seguridad. También podría relacionarse con el Bug #5 (`conversation_phase` a veces no se actualiza) si el `[CTX: phase=...]` que ve el modelo no refleja la fase real en ese turno — sin verificar aún con datos crudos de `n8n_chat_histories` del turno exacto. **Pendiente:** pedir al Agente n8n el contenido exacto (CTX + fase autoreportada) de los turnos alrededor del loop en staging, y buscar más casos en prod con el mismo patrón para dimensionar el impacto. **Distinto del Bug #2:** el caso 2205 SÍ tenía `qid` válido (no es el patrón `qid=null/fallback` de sesiones sin match) — es un bug de clasificación de intención independiente, no de session_id. | n8n | 🔴 Crítico — bloquea conversiones reales y bloquea la validación E2E del Bug #10 |

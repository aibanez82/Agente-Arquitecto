...
- **Una medición del presente no acredita el pasado (24-25 ago):** al explicar un incidente pasado con
  la configuración **actual** de un sistema mutable (scheduler, flag, config var, grafo n8n), comprobar si esa
  configuración pudo cambiar entremedias. Si el sistema no guarda historia, no se puede afirmar nada del pasado
  desde ahí. El ámbito de una afirmación lleva **fecha**, no solo ruta. Historia: `docs/architecture/convenciones-origen.md`.
...
- **Gitflow en TODOS los repos, no solo en el de Juan (14 ago):** el código se desarrolla en `feature/…`, `fix/…` o `docs/…` (kebab-case) **sacadas de `stg`**, y entra a `stg` **por merge**, nunca por commit directo. `main` describe lo que corre en PROD y solo se toca por promoción desde `stg`. Commits pequeños y revisables, árbol limpio, SHA publicado, y gates verdes **antes** de integrar. Aplica a `Agente-n8n`, `Dashboard_seguroautoqualitas` y `HYL-WAI`. **`Agente-Arquitecto` no tiene `stg`** (Alberto, 28 ago): sus ramas `docs/…` salen de `main` y entran por PR. **Única excepción, y es de canal no de código:** `handoffs/`, `dudas/` e `informes/` van directos a `main` porque ahí los vigilan los monitores de los ejecutores — mover eso rompería la comunicación, no la mejora.

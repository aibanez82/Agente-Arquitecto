...
- **Una guarda se juzga por su mecanismo, no por su intención (23 ago):** `information_schema` **filtra por privilegios**, así que con un rol restringido «no existe» y «existe sin grants» se ven **idénticos** — una precondición escrita sobre él **falla abierta** y lee la ausencia como conformidad. Para comprobar catálogo: `pg_catalog` (`pg_attribute`, `pg_class`) y `to_regclass`, que no filtran. Y que la guarda exija lo esperado **en positivo**: si solo comprueba que nada incumple, cero filas la pasa.

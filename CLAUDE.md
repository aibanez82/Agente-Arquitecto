...
| `whatsapp_sessions`/`whatsapp_sessions_archive` con columnas de tiempo naive (candidato a causa del Issue #74) | ✅ Resuelto — migradas a `timestamptz` en PROD, verificado en vivo por el Arquitecto 11 jul. Detalle: `docs/architecture/timezone.md` |
| Issue #74 (`aguayo-co/HYL-WAI`) — follow-up 15 min dejó de enviarse desde 2026-06-30 ~21:11 UTC | ⏳ Causa raíz sin determinar, pero el candidato fuerte (timestamps naive en `whatsapp_sessions`) ya se corrigió en PROD (11 jul) — **pendiente re-verificar si el follow-up ya se está enviando de forma confiable** antes de seguir buscando otra causa |

...
| `N8N_TOKEN` con valor real hardcodeado como default en `qualitas/views.py:1291` (`enviar_webhook_whatsapp`; reverificado en `main` 27 jul — el endpoint validador `:1041` ya es env-only) | 🔴 **Issue #130 `HYL-WAI` abierto 27 jul** (quedó huérfano al cerrarse #110). Pedido a Juan: quitar default, rotar token, coordinar con Alberto el cambio simultáneo en credenciales n8n. Ver `docs/iniciativas/entorno-pruebas-staging.md` |
...
| Exponer `fecha_inicio` en emisión — Issue #114 `HYL-WAI` | ✅ **En PROD 27 jul** (PR #125, release v331 = `43bfaf2`, migraciones 0047–0052 verificadas). Máx. 30 días de diferimiento. Falta solo lado n8n (promoción selectiva renovación+`Issue Policy`): handoff 27 jul en `Agente-n8n/handoffs/`. Desbloquea M47/M48 |

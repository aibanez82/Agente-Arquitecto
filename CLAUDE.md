...
- **No propongas instrumentación nueva: el plan es retirarla.** Canarios, contadores, flags de medición, paths temporales — **no se proponen, ni «desactivados por si acaso»**. `HYL-WAI#146` (C9 — Contracción) ordena eliminar paths/flags temporales con issue, inventario y evidencia: lo que se añade hoy es deuda que S5 retira con expediente. Si una propuesta se queda sin forma de comprobar si basta, **eso se dice como límite conocido**; no se tapa midiendo. Alcance: instrumentación **nueva**. Sobre la que ya existe (monitores de sesión, gates) esta regla no dice nada — no se crea más, y lo vivo se retira cuando lo ordene su plan, no por esta.

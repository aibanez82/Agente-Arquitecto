...
- **Orden de arranque explícita en handoffs (9 ago):** un handoff publicado es **contenido**, no orden de ejecutar. Lleva línea `Orden de arranque:` con valor `PENDIENTE — no ejecutar` al publicarse; se cambia a `DADA por <quien ordena>, <hora>` en **commit propio**. Sin esa línea, o con `PENDIENTE`, el ejecutor NO ejecuta. Nació de escribir «retransmitido por Alberto» como fórmula en 8 handoffs: un estado esperado escrito como observado, y encima el que decide si se puede ejecutar. Precisiones (9 ago, tras incumplirla el propio Arquitecto): **nunca se publica ya en `DADA`**, ni siquiera por el Arquitecto — una línea escrita en el commit que crea el fichero es indistinguible de una fórmula y pierde su valor de señal. Quién puede ordenar: **Alberto siempre**; el **Arquitecto solo** para trabajo estrictamente offline (sin efecto vivo, sin tocar artefacto acreditado, sin rama fusionada), y aun así en commit propio y posterior. Y la orden **no se marca sobre un traslado de terceros**: la escribe quien observó el hecho, o se confirma antes con quien la dio.

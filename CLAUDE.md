...
> Exportar y hacer commit aquí cada vez que se modifique un workflow en producción — mientras el backup automático siga caído (ver Pendientes), es la única red de seguridad.
...
**Seguimiento automático de leads estancados:** 7 checkpoints, hasta 3 reintentos. ✅ Todo confirmado en PROD, corriendo en dry-run (`ENABLED=true`, `DRY_RUN_DEFAULT=true`). **Falta solo:** filtro de horario (9am-8pm CDMX, decidido, sin construir) y luego una prueba de envío real controlada — no activar envío real antes del filtro (casi-incidente 19 jul: recordatorio a las 11pm). Detalle: `docs/iniciativas/seguimiento-leads-estancados.md`.

...
**Seguimiento automático de leads estancados (15-16 jul):** 7 checkpoints derivados en vivo, hasta 3 reintentos, sin campo de estado nuevo. **Django y n8n completos y verificados en STG** (16 jul) — `session_id` sin fallback, validación dual para no romper "Tomar conversación" del Dashboard, idempotencia real, 5 pruebas verificadas en vivo. Único pendiente: activar (falta que Alberto comparta el token con Juan). (Descartado: el scheduler de 15 min real es de 4 min y solo aplica si el cliente no respondió nada — sin solapamiento.) Detalle: `docs/iniciativas/seguimiento-leads-estancados.md`.

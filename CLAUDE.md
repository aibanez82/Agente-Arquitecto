...
| 15 | **El número de WhatsApp de test (staging) NO está aislado — comparte `phone_number_id` (`1154577517746231`, número visible `17377323515`) con la instancia de n8n de PRODUCCIÓN.** Confirmado por el Agente n8n (7 jul) cruzando ejecuciones de ambas instancias: 13 ejecuciones en PROD durante la prueba E2E de hoy (workflow `BtOaZm7WlZT-24V7hqCnF`, ids 2162-2177) con timestamps casi idénticos (diferencia de ~100ms) a las 15 ejecuciones equivalentes en STAGING (workflow `dNqtM20ij6ecZYAX`, ids 19-33) — Meta está entregando el mismo evento de webhook a las dos Apps/instancias. **Confirmado escrito en la BD REAL de prod** (credencial `Postgres account`): `n8n_chat_histories` (ruido de conversación de prueba) y `whatsapp_sessions` (`Update Phase in DB` corrió contra la sesión real de prod para el número de prueba). **Buena noticia — contenido:** `Issue_Policy` NUNCA se ejecutó en prod en ninguna de las 13 ejecuciones — el `quotation_id` de prueba (1587) no existe en el Django real de prod, así que cayó a fallback ("no tengo una cotización válida") antes de llegar a emisión. **0 pólizas ni llamadas a Quálitas de producción disparadas.** Afecta también a un segundo número de prueba (Juan, `573107696237` — mismo patrón, prefijo `57` = Bug #2 coincidiendo con esto). **No se tocó ni borró nada de prod** — la sesión `525551074144` se dejó intacta como evidencia. **Bloquea toda prueba nueva de staging hasta que Juan aísle de verdad el número de test en su propia WABA** (el App nuevo que creó, per `docs/2026-07-06-mensaje-juan-meta-app-staging.md`, no logró el aislamiento esperado). **Diagnóstico profundizado 7 jul vía Graph API directa** (con `STG_META_ACCESS_TOKEN`/`STG_META_APP_SECRET`, guardados en `.env.local`): número de PROD (`1028815256982638`) tiene override de webhook explícito a nivel de número (`n8n.srv1325340.hstgr.cloud`); número de TEST (`1154577517746231`, App `hyl-wai-stg` id `4539428293006817`) **no tiene override a nivel de número, y `/subscriptions` de la App está vacío** — descarta una App-level webhook mal apuntada. **Hipótesis de WABA compartida DESCARTADA (7 jul, con los WABA IDs reales):** WABA de staging (`27763806206640265`, `subscribed_apps` = solo `hyl-wai-stg`) y WABA de producción (`2418053602347168`, `subscribed_apps` = solo `Aguayo IA`) son distintas, cada una con un único App suscrito — a nivel de Meta se ve limpio, sin suscripción cruzada visible. **Esto pone en duda que la causa sea un cruce de infraestructura Meta/n8n** — el contenido sí apareció en prod (confirmado por SQL), pero la explicación de "Meta duplica el webhook" ya no está sostenida por evidencia. **Pendiente:** pedí al Agente n8n el payload crudo (`phone_number_id`/WABA id dentro del JSON del webhook) de 2-3 de las 13 ejecuciones de prod para saber si de verdad llegó un evento de staging a prod, o si el origen es otro (p. ej. Alberto escribiendo directo al número real de producción en un thread separado, algo que ya ha hecho antes con el mismo teléfono). Handoff: `docs/2026-07-07-handoff-agente-n8n-verificar-payload-crudo-bug15.md`. **NO se le mandó nada nuevo a Juan** hasta confirmar esto. | Meta/Infraestructura n8n (sin confirmar) | 🟡 En investigación — hipótesis original de infraestructura compartida no sostenida por los datos de WABA |

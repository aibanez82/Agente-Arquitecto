...
| ~~`NumeroPruebaWhatsapp`~~ | — | Sigue sin existir en producción (reconfirmado 20 jul), pero ya no importa: `normalize_whatsapp_phone` (`qualitas/whatsapp_conversations.py`, PR de Juan) cae siempre a `52` por try/except ante tabla inexistente. Bug #2 cerrado. |
...
**Seguimiento automático de leads estancados (15-19 jul):** 7 checkpoints, hasta 3 reintentos. ✅ Tabla+fixture (21 filas), interpolación, guard de `status` cerrado, workflow robusto `Retomar Conversacion` y Scheduler (cada 5 min, Advanced) — todo confirmado en PROD, corriendo en dry-run (`ENABLED=true`, `DRY_RUN_DEFAULT=true`, estado seguro). **Falta solo:** filtro de horario (9am-8pm CDMX, decidido, sin construir) y luego una prueba de envío real controlada — bloqueado por el casi-incidente de la madrugada del 19 jul (casi se manda un recordatorio real a las 11pm). Detalle: `docs/iniciativas/seguimiento-leads-estancados.md`.
...
| Política de backup automático de workflows n8n | 🔴 Deshabilitado desde 6 jul (401, key vencida). Re-exportado a mano 22 jul (drift había llegado a 107 nodos PROD vs 61 local). Reactivar tras rotar la key. Detalle: `docs/architecture/backup-policy-n8n.md` |
...
| `N8N_TOKEN` con valor real hardcodeado como default en `qualitas/views.py:905` (confirmado también en `main`, 16 jul) | 🔴 Sube de prioridad (22 jul): ya gatea en PROD el acceso a PDFs de cotización reales (feature "entrega quick reply" resultó estar activa). Mover a solo-env y rotar, pedir a Juan. Ver `docs/iniciativas/entorno-pruebas-staging.md` |
| Cómo saber con certeza si un cliente pagó la póliza | ✅ Resuelto operativamente (26 jul) — Agente Conciliación con cron diario contra el portal Q 360, escribe `conciliacion_pagos`. Pendiente de negocio: qué hacer con `VENCIDO`/`CANCELADO` (ver reporte 26 jul). Ver `docs/architecture/estatus-pago-qualitas.md` |

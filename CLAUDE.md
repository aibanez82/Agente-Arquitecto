...
**Seguimiento automático de leads estancados (15-16 jul):** 7 checkpoints derivados en vivo, hasta 3 reintentos, sin campo de estado nuevo. Django implementado y verificado en STG (16 jul). **Bloqueante encontrado:** el nodo `Retomar Conversacion` de n8n no cumple el contrato de `session_id` que el propio Juan especificó — usa `conversation_id` con prioridad cuando debería usar siempre `session_id`, confirmado con 3 sesiones reales en STG donde ambos difieren. Pendiente: Agente n8n corrija el nodo. (Descartado: el scheduler de 15 min real es de 4 min y solo aplica cuando el cliente no ha respondido nada — poblaciones mutuamente excluyentes, sin solapamiento.) Detalle: `docs/iniciativas/seguimiento-leads-estancados.md`.

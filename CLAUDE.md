...
| Migración KB del bot a RAG real (pgvector + OpenAI embeddings) | ✅ En PROD desde 18 jul (`kb_chunks` 118 filas + fallback `doc_chunks` 152), Alberto pidió el deploy directo. E2E real confirmado en PROD para RAG general, M36/M38 y bug del Intent Router (issue #46, cerrado). Pendiente: E2E de `doc_chunks` fallback — el caso de prueba original quedó invalidado (`kb_chunks` de PROD ya cubre ese tema), retry en curso. Detalle: `docs/iniciativas/2026-07-17-migracion-rag-kb-pgvector-design.md` |

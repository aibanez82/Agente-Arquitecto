...
| Migración KB del bot a RAG real (pgvector + OpenAI embeddings) | ✅ En PROD desde 18 jul (`kb_chunks` 119 filas + fallback `doc_chunks` 152), Alberto pidió el deploy directo. E2E real confirmado para RAG general y M36/M38; `kb_chunks.id=38` (contenido engañoso) corregido y cerrado. Bloqueante activo: bug crítico — `RAG IA Agent` mandó sintaxis cruda de tool-call a un cliente real (issue #47), guard defensivo en curso, bloquea el cierre de E2E de `doc_chunks`. Detalle: `docs/iniciativas/2026-07-17-migracion-rag-kb-pgvector-design.md` |

...
**Orden de contención barrera A — llamadores primero, verificado contra el grafo congelado + re-verificar contra el grafo vivo:** Main (1) → los 4 con ingress propio (2–5: Atención Humana, Payment, Metepec Liberar, Retomar) → los **dos callees puros sin ingress** (6–7: Issue Policy Guard, METEPEC Registrar) **los últimos**. Razón: gatear un callee con su llamador vivo hace que el llamador invoque un sub cerrado → el bot se rompe por dentro. `ordenPorDependencias` lo computa; se re-verifica en la ventana.

...
- 🔴 **`doc_chunks` fallback: sigue SIN verificación E2E real en PROD — 1er intento inconcluso, no invalidante.** Primer intento (18 jul, casos Auto Sustituto + control de pago) no disparó `search_doc_corpus` en ninguno de los dos — verificado contra `n8n_chat_histories` real: la pregunta de Auto Sustituto ya se había hecho antes en la misma sesión (reusó respuesta de memoria) y, más importante, **el `kb_chunks` de PROD ya tiene contenido curado real y correcto sobre Auto Sustituto** (chunk verificado: exclusiones de gasolina/multas/deducible/robo parcial — no es alucinación, es contenido genuino) — quedó cubierto por Tier 1 antes de llegar a necesitar el fallback. El caso de prueba se diseñó contra el `kb_chunks` viejo de STG, que no tenía ese contenido; el de PROD (reconstruido el 18 jul desde el `Code` node real) sí. Retry en curso con un tema que sea hueco real verificado contra el `kb_chunks` de PROD específicamente, evitando la sesión ya contaminada. Handoff: `Agente-n8n:handoffs/2026-07-18-handoff-retry-e2e-doc-chunks-prod.md`.

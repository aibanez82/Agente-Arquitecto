...
**✅ Bug crítico cerrado — `RAG IA Agent` mandó sintaxis cruda de tool-call a un cliente real (issue #47).** Encontrado durante el 3er intento de Tarea 2 (tema "Agravación de Riesgo"): 2 ejecuciones consecutivas (2935, 2936) devolvieron texto `<invoke name="search_knowledge_base1">...` tal cual al cliente en vez de una respuesta — confirmado por trace, ningún tool se ejecutó de verdad. Hipótesis inicial de Agente n8n (`contextWindowLength` subido a 60) **descartada por el Arquitecto, verificado en vivo**: PROD sigue en `contextWindowLength: 20`, esa mitigación (issue #45) nunca se promovió desde STG. Causa más plausible: la sesión de pruebas (`525551074144`) acumuló 56 mensajes en un día reusada para todo el trabajo de hoy — densidad/repetición/inyecciones sintéticas (`[MEDIA_NO_SOPORTADA]`, `[CTX:...]`) atípicas de cualquier conversación real, no confirmado al 100%. El patrón `"Calling <tool> with input: {json}"` en sí es normal y preexistente (confirmado en sesiones reales del 17 jul), no es la causa por sí solo. Fix: guard defensivo antes de `Send message` (nodos `Detect Failed Tool Call`/`Detect Failed Tool Call1`, uno por agente) — desplegado en STG y PROD, verificado por el Arquitecto vía API (wiring correcto, lógica sin falsos positivos sobre el patrón legítimo). Certificado y cerrado en `qualitas-issues#47`.

**✅ Tarea 2 (E2E `doc_chunks` fallback) — CERRADA, 3er intento limpio.** Tema aislado "Agravación de Riesgo" (glosario, sin relación con coberturas): ejecución 2964 llamó `search_knowledge_base1` Y `search_doc_corpus`, respuesta verificada palabra por palabra contra el PDF real (Cláusula 8ª "Obligaciones del Asegurado" — "debe comunicar... dentro de 24 horas... cesarán de pleno derecho las obligaciones de la Compañía"). Controles limpios: 2965 (pregunta que `kb_chunks` ya resuelve, no activó el fallback) y 2966 (fuera de dominio, interceptado por `Intent Router`). Los 2 intentos previos fallidos no eran un problema de diseño del trigger — solo hacía falta un tema genuinamente aislado semánticamente. El guard corrió en las 3 ejecuciones sin falsos positivos.

## Estado final (18 jul 2026)

Migración RAG completa en PROD: `kb_chunks` (119 filas) + `doc_chunks` fallback (152 filas), ambos con E2E real confirmado. Los 2 bugs encontrados durante la validación (`kb_chunks.id=38` y el tool-call crudo) quedaron corregidos y cerrados el mismo día. Sin pendientes urgentes de esta iniciativa.

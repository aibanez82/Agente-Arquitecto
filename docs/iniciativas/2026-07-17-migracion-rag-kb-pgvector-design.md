...
**🔴 Hallazgo colateral, prioridad alta, ya resuelto en el handoff — `kb_chunks.id=38` daba respuesta engañosa en PROD.** Encontrado durante el intento 2 de arriba: a la pregunta real "se me ponchó una llanta... ¿me mandan ayuda aunque no esté de viaje?", el bot recuperó el chunk de "coberturas accesorias de pago opcional" y le dijo al cliente que necesita contratar "Llantas y Rines" para recibir ayuda — cuando el Auxilio Vial por ponchadura (Cláusula 6.1, 5 eventos/año, radio 80 km) **ya viene incluido** en la Cobertura Limitada base (confirmado también por `kb_chunks.id=36`). Verificado por el Arquitecto directo contra el PDF de Condiciones Generales (mismo método que M36) antes de especificar el fix: chunk nuevo dedicado a Auxilio Vial/ponchadura + nota aclaratoria agregada a `id=38` — pendiente de aplicar por Agente n8n, luego re-probar la pregunta real en PROD.

...
| 19 ago 2026 · forma | 23.605 → 23.578 | 13 §§, 0 rutas rotas | **Segunda pasada del mismo día, de forma.** Seis convenciones de git dispersas → un bloque «Git y ramas», y al agruparlas salió que la regla listaba mal los repos: decía `Agente-Arquitecto` (que **no tiene `stg`**) y omitía `Agente-Conciliacion` (que sí) — verificado con `ls-remote` en los siete clones. La regla de comunicación estaba duplicada íntegra en «Identidad» y en «Convenciones»: una sola, y la derogación del 10 ago —que `convenciones-origen.md` **no** registraba— escrita allí. «No ejecuto nada» → «no toco sistemas vivos», que es la frontera real. `conversation_phase` reconciliada: `greeting` stuck es del bot (`#56`), `completed` lo escribe *Payment Confirmation* y por eso el workaround del Bug #7 se sostiene. Env vars de Vercel → `docs/architecture/variables-entorno-vercel.md`. Narrativa inline fuera; quedan tres fechas, y las tres son marcas de *cuándo se midió*. Casi no encogió: lo que ahorró la fusión se lo comió decir bien lo que estaba mal dicho |
| 19 ago 2026 · punteros muertos | 23.195 → 23.605 | — | **Creció a propósito: tres punteros mentían.** (1) El tablero artifact, RETIRADO el 9 ago por la propia convención, seguía citado en dos sitios como «estado del día» — ahora `#132` y el doc de iniciativa. (2) «Promoción a PROD de `fecha_inicio` → `qualitas-issues#66`»: el #66 real es el guard de los 30 días de diferimiento, otro defecto; el bloqueante Django (`HYL-WAI#114`) está cerrado desde el 24 jul y la promoción **no tiene issue** — queda dicho, con el estado en n8n PROD marcado como no verificado. (3) La URL de la API de n8n aparecía desnuda, sin decir que era PROD, con la de STG en otra sección: ahora las dos juntas y etiquetadas. Los 15 issues citados en el archivo se verificaron uno a uno contra `gh`: todos abiertos |
